...
public:
	...
	bool	ReadVideo();
	void	CalcMotion();
	void	MAD(std::vector<std::vector<float>>& lastFrame, std::vector<std::vector<float>>& curFrame, std::vector<std::vector<std::pair<int, int>>>& motionVec, int k);
	...
	void	dct(std::vector<unsigned char>& block, std::vector<int>& coeff, std::vector<std::vector<float>>& temp, int n);
	void	DCT(std::vector<std::vector<std::vector<int>>>& tmpImg, unsigned char* iData, std::vector<bool>& iBlockType);
