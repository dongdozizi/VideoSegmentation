...
void MyVideo::MAD(std::vector<std::vector<int>>& lastFrame, std::vector<std::vector<int>>& curFrame, std::vector<std::vector<std::pair<int, int>>>& motionVec, int k) {
	...
	for (int i = 0; i < height; i += 16) {
		for (int j = 0; j < width; j += 16) {
			...
			for (const auto& offset : offsets) {
				...
				for (int r = 0; r < h; r++) {
					for (int s = 0; s < w; s++) {
						tmp += std::abs(curFrame[i + r][j + s] - lastFrame[p + i + r][q + j + s]);
						if (tmp > sum) break;
					}
					if (tmp > sum) break;
				}
...
void MyVideo::CalcMotion() {
	...
	std::vector<std::vector<int>> curFrame(Height, std::vector<int>(Width, 0)), lastFrame(Height, std::vector<int>(Width, 0));
